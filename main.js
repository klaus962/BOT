...
async function bootstrapSystem() {
  ...
  try {
    ...
    play = (filePath) => {
      if (isSoundEnabled()) soundModule.play(filePath);
    };
    playError = () => {
      if (isSoundEnabled()) soundModule.playError();
    };
    playLogout = () => {
      if (isSoundEnabled()) soundModule.playLogout();
    };
  } catch (e) {
    logger = console;
    play = () => {};
    ...
    if (fs.existsSync(settingsPath)) {
      ...
      if (fs.existsSync(themeFolder)) {
        ...
        if (jsFiles.length > 0) {
          ...
          themeData = {
            asciiArt: themeArt.asciiArt || '',
            soundPath,
            themeColor: match ? match[1] : 'FF6BFF'
          };
        }
      }
    }
